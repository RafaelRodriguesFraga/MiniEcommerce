...
public class SwaggerDocumentationOperationFilter : IOperationFilter
{
    public void Apply(OpenApiOperation operation, OperationFilterContext context)
    {
        ...
        var getMethod = repositoryType.GetMethod("Get", BindingFlags.Public | BindingFlags.Static | BindingFlags.FlattenHierarchy);

        if (getMethod == null)
        {
            throw new InvalidOperationException($"Could not find a static public method named 'Get' in {repositoryType.Name}.");
        }

        var parameters = getMethod.GetParameters();
        if (parameters.Length != 1)
        {
            throw new InvalidOperationException($"The 'Get' method in {repositoryType.Name} must have exactly one parameter.");
        }
